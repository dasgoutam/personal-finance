<script lang="ts">
	...
	export let grouping: { category: string; total: number }[]
	...
	$: currentMonthIndex = parseInt(month, 10) - 1

	$: grandTotal = grouping.reduce((sum, e) => sum + e.total, 0)
	$: rows = grouping.map((e, i) => ({
		category: e.category,
		total: e.total / 100,
